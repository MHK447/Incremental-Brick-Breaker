...
#pragma warning restore 0649
		...
		public void UpdatePosition( bool immediately )
		{
			...
			if( horDistance < vertDistance )
			{
				...
				pos.y = Mathf.Clamp( pos.y, canvasHeight * 0f + halfSize.y, canvasHeight  - halfSize.y );
			}
			else
			{
				if( distToBottom < distToTop )
					pos = new Vector2( pos.x, canvasHeight * 0f + halfSize.y );
				else
					pos = new Vector2( pos.x, canvasHeight  - halfSize.y );

				pos.x = Mathf.Clamp( pos.x, canvasWidth * 0f + halfSize.x, canvasWidth - halfSize.x );
			}
