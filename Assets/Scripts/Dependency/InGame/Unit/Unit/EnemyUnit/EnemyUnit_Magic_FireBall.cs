...
public class EnemyUnit_Magic_FireBall : EnemyUnit
{
  [SerializeField]
    ...
    public override void Attack()
    {
        ...
        float blockDistance;
        bool isBlockInRange = IsBlockTargetInRange(out blockDistance);
        ...
        if (!isTargetingBlock)
        {
            ...
            // PlayerUnit이 있으면 우선 후보로 계산
            float unitDistance = float.MaxValue;
            bool hasValidUnitTarget = false;
            ...
            if (UnitTarget != null && UnitTarget.gameObject.activeInHierarchy && UnitTarget.CurState != StateType.Dead)
            {
                ...
                unitDistance = Vector3.Distance(currentPosXZ, targetPosXZ);

                hasValidUnitTarget = unitDistance <= InfoData.AttackRange;
            }

            // 블록(성)도 사정거리 안이면 블록을 우선 타겟으로 전환
            bool shouldTargetBlock = isBlockInRange && (!hasValidUnitTarget || blockDistance <= unitDistance);

            if (shouldTargetBlock)
            {
                UnitTarget = null;
                targetTransform = BlockTarget.transform;
                isTargetingBlock = true; // 성 공격 모드 활성화
            }
            else if (hasValidUnitTarget)
            {
                targetTransform = UnitTarget.transform;
            }
            else if (isBlockInRange && BlockTarget != null)
            {
                UnitTarget = null;
                targetTransform = BlockTarget.transform;
        ...
        {
            // 성 공격 모드일 때는 성만 공격
            if (isBlockInRange && BlockTarget != null)
            {
                targetTransform = BlockTarget.transform;
            }
            else
    ...
    private bool IsBlockTargetInRange(out float distanceToBlock)
    {
        distanceToBlock = float.MaxValue;
        ...
        distanceToBlock = Vector3.Distance(currentPosXZ, blockPosXZ);
        return distanceToBlock <= InfoData.AttackRange;
    }
}
