...
public class TutorialSystem
{
    ...
    public void Dispose()
    {
        foreach (var map in scenarioMap)
        {
            if (map.inst != null)
                Addressables.ReleaseInstance(map.inst);
        }
        scenarioMap.Clear();

        foreach (var activeMap in Active_ScenarioMap)
        {
            if (activeMap != null)
            {
                if (!Addressables.ReleaseInstance(activeMap))
                    GameObject.Destroy(activeMap);
            }
        }
        Active_ScenarioMap.Clear();
