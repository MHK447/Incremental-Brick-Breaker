	...
	public class TableCreatorWindow : EditorWindow
	{
		...
		Func<int, string> SearchType = (index) =>
		{
			...
			foreach (var row in cachedDataList)
			{
				...
				// 에러 로그 추가: 컬럼 인덱스 범위 체크
				if (index >= row.Count)
				{
					Debug.LogError($"[TableCreator] 컬럼 인덱스 에러 발생!");
					Debug.LogError($"  - 요청된 인덱스: {index}");
					Debug.LogError($"  - 현재 행의 컬럼 수: {row.Count}");
					Debug.LogError($"  - 헤더 컬럼명: {cachedDataList[0][index]}");
					Debug.LogError($"  - 데이터 행 번호: {iterCnt}");
					Debug.LogError($"  - 현재 행 데이터: [{string.Join(", ", row)}]");
					return "string"; // 기본값 반환
				}
